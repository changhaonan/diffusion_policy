...
class ControlDiffusionTransformerHybridImagePolicy(BaseImagePolicy):
    def __init__(
        self,
        shape_meta: dict,
        ...
        **kwargs,
        ...
        self.mask_generator = LowdimMaskGenerator(action_dim=action_dim, obs_dim=0 if (obs_as_cond) else obs_feature_dim, max_n_obs_steps=n_obs_steps, fix_obs_steps=True, action_visible=False)
    ...
    def get_optimizer(self, transformer_weight_decay: float, obs_encoder_weight_decay: float, learning_rate: float, betas: Tuple[float, float]) -> torch.optim.Optimizer:
